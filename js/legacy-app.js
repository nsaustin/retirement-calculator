...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function generatePdfReport() {
        ...
        // Add title and date
        doc.setFontSize(22);
        doc.setTextColor(58, 110, 165); // Primary color
        doc.text('Retirement Readiness Report', 105, 20, { align: 'center' });
        ...
        doc.setDrawColor(58, 110, 165);
        doc.setLineWidth(0.5);
        doc.line(20, 35, 190, 35);
        
        // Get user input data
        const currentAge = document.getElementById('current-age').value || 'N/A';
        const netWorth = formatCurrency(parseFloat(document.getElementById('net-worth').value) || 0);
        ...
        doc.text(`Current Age: ${currentAge}`, 20, 55);
        doc.text(`Current Net Worth: ${netWorth}`, 20, 62);
        
        // Get financial parameters
        const withdrawalRate = document.getElementById('withdrawal-rate-value').textContent;
        const returnRate = document.getElementById('return-rate-value').textContent;
        const inflationRate = document.getElementById('inflation-rate-value').textContent;
        
        // Financial parameters section
        doc.setFontSize(16);
        doc.setTextColor(58, 110, 165);
        doc.text('Financial Assumptions', 20, 75);
        
        doc.setFontSize(12);
        doc.setTextColor(0, 0, 0);
        doc.text(`Annual Withdrawal Rate: ${withdrawalRate}`, 20, 85);
        doc.text(`Expected Annual Return: ${returnRate}`, 20, 92);
        doc.text(`Expected Inflation Rate: ${inflationRate}`, 20, 99);
        
        // Results section
        doc.setFontSize(16);
        doc.setTextColor(58, 110, 165);
        doc.text('Retirement Analysis Results', 20, 115);
        
        // Get results data
        const totalMonthlyExpenses = document.getElementById('total-monthly-expenses').textContent;
        const annualExpenses = document.getElementById('annual-expenses').textContent;
        const requiredSavings = document.getElementById('required-savings').textContent;
        const realReturnRate = document.getElementById('real-return-rate').textContent;
        const ssPercent = document.getElementById('ss-coverage-percent') ? 
            document.getElementById('ss-coverage-percent').textContent : 'N/A';
        
        // Results grid
        doc.setFontSize(12);
        doc.setTextColor(0, 0, 0);
        doc.text(`Monthly Expenses: ${totalMonthlyExpenses}`, 20, 125);
        doc.text(`Annual Expenses: ${annualExpenses}`, 20, 132);
        doc.text(`Required Savings: ${requiredSavings}`, 20, 139);
        doc.text(`Real Return Rate: ${realReturnRate}`, 20, 146);
        doc.text(`Social Security Coverage: ${ssPercent}`, 20, 153);
        ...
        // Get insight text
        const insightText = document.getElementById('retirement-insight').innerText;
        const moneyLastingText = document.getElementById('money-lasting').innerText;
        const inflationImpactText = document.getElementById('inflation-impact').innerText;
        
        // Add chart from canvas as image (if available)
        try {
            ...
            if (chartCanvas) {
                ...
                doc.setFontSize(16);
                doc.setTextColor(58, 110, 165);
                doc.text('Retirement Projection Chart', 105, 20, { align: 'center' });
                doc.addImage(chartImg, 'PNG', 15, 30, 180, 100);
                doc.setFontSize(10);
                doc.setTextColor(102, 102, 102);
                doc.text('This chart shows your projected net worth over time based on your inputs', 105, 140, { align: 'center' });
            }
        } catch (e) {
            console.error('Could not add chart to PDF', e);
        }
        
        // Format and add insights
        doc.setFontSize(10);
        doc.setTextColor(0, 0, 0);
        ...
        doc.setFontSize(12);
        doc.setTextColor(58, 110, 165);
        doc.text('Retirement Readiness:', 20, yPos);
        doc.setTextColor(0, 0, 0);
        doc.setFontSize(10);
        yPos = addWrappedText(insightText.replace(/<\/?span.*?>/g, ''), 20, yPos + 8, 170, 5);
        
        doc.setFontSize(12);
        doc.setTextColor(58, 110, 165);
        doc.text('Portfolio Longevity:', 20, yPos + 5);
        doc.setTextColor(0, 0, 0);
        doc.setFontSize(10);
        yPos = addWrappedText(moneyLastingText.replace(/<\/?strong>/g, ''), 20, yPos + 13, 170, 5);
        ...
        // Check if SS impact section exists and is visible
        const ssImpactSection = document.getElementById('ss-impact-section');
        if (ssImpactSection && ssImpactSection.style.display !== 'none') {
            const ssImpactText = document.getElementById('ss-impact-text').innerText;
            
            doc.setFontSize(12);
            doc.setTextColor(58, 110, 165);
            doc.text('Social Security Impact:', 20, yPos + 5);
            doc.setTextColor(0, 0, 0);
            doc.setFontSize(10);
            yPos = addWrappedText(ssImpactText.replace(/<\/?br>|<\/?strong>/g, ' '), 20, yPos + 13, 170, 5);
        }
        ...
        for (let i = 1; i <= pageCount; i++) {
            ...
            doc.text('Created with Retirement Calculator', 105, 290, { align: 'center' });
